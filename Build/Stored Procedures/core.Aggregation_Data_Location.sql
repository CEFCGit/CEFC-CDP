...
-- Description:	Create stored procedure to get the relevant commitment amounts based on Aggregation File Date split by Location
-- ==========================================================================

CREATE or alter  procedure [core].Aggregation_Data_Location @file_date datetime
as 
	...
	as 
		...
		max(l.Update_From_TS) l_Update_From_TS,
		max(l.Update_to_TS) l_Update_To_TS,  
	    prjd.ID_Project,
		prjd.Project_Name,
		prjd.Description,
	    ...
	    on prjd.ID_Project = l.id_project
		...
		and EOMONTH(@file_date) >= convert(datetime,l.Update_From_TS)
		and EOMONTH(@file_date) <= convert(datetime,isnull(l.Update_to_TS,@local_date))
    ...
    select af.[ID_Project]
	  ...
	  ,ad.ABS_Division
	  ,ad.ABS_Subdivision
	  ,t.Amt_CEFC*t.Percent_of_Project_Amt as Amt_CEFC
	  ,t.Project_Name
	  ,t.cOrganisations
	  ...
	  ,t.Loc_Postcode
	  ,t.Loc_Electorate
	  ,t.Percent_of_Project_Amt
	  ,t.Percent_of_Project_tCO2_savings
